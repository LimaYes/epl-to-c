...
public class ASTBuilder {
    ...
    private static void push_exp(Primitives.STATE state, Primitives.AST exp) {
        ...
        for (i = state.ast_func_idx; i < state.stack_exp.size(); i++) { //todo: why was <= here before??
            ...
            while (exp.right != null) {
                ...
                if (exp.right.left != null) {
                    ...
                    } else if ((exp.right.left.type == NODE_CALL_FUNCTION) && exp.right.left.svalue != null && !CodeConverter.strcmp(exp.right.left.svalue, "verify")) {
                        if (i != state.ast_main_idx) {
            ...
            while (ast_ptr != null) {
                ...
                if (downward) {
                    ...
                    if (ast_ptr.type == NODE_CALL_FUNCTION) {
                        ...
                        if (ast_ptr.uvalue == 0) {
                            for (i = 0; i < state.stack_exp.size(); i++) {
                                if ((state.stack_exp.get(i).type == NODE_FUNCTION) && !CodeConverter.strcmp(state.stack_exp.get(i).svalue, ast_ptr.svalue))
                                    ast_ptr.uvalue = i;
                            }
                        }
                        ...
                        for (i = 0; i < call_stack.size(); i++) {
                            if (ast_ptr.svalue != null && call_stack.get(i).svalue != null && !CodeConverter.strcmp(ast_ptr.svalue, call_stack.get(i).svalue)) {
